...
describe("POST /skill/corps/list", () => {
  it("lists known corps codes", async () => {
    const res = await app.request("/skill/corps/list", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(makeSkillRequest("부대 조회")),
    });
...
describe("POST /skill/corps/change", () => {
  it("sets corps from the action param", async () => {
    const res = await app.request("/skill/corps/change", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(makeSkillRequest("부대 변경 1570", { corps: "1570" })),
    });
  ...
  it("falls back to parsing the utterance when no param is given", async () => {
    await app.request("/skill/corps/change", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(makeSkillRequest("부대 atc로 바꿔줘")),
    });
  ...
  it("rejects an unrecognized corps code", async () => {
    const res = await app.request("/skill/corps/change", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(makeSkillRequest("부대 변경 9999")),
    });
